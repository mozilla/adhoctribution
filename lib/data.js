 ...
 * @return {JSON}
 */
exports.getContributorCounts = function getContributorCounts(date, teamname, bucket, callback) {
  var counts = {};
  ...
  pool.getConnection(function connectionAttempted(err, connection) {
    ...
    if (err) {
      ...
      var dataBucket = connection.escape(bucket);
      ...
      async.parallel({
          last_year: function (callback) {
            ...
            connection.query('SELECT DISTINCT contributor_id FROM contributions ' +
              ...
              ' AND mofo_team = ' + mozTeam + ' AND data_bucket = ' + dataBucket,
...
exports.recentlyLogged = function recentlyLogged(email, callback) {
  pool.getConnection(function (err, connection) {
    if (err) {
      ...
      connection.query('SELECT * FROM contributions WHERE logged_by = ? ORDER BY logged_on ASC LIMIT 20;', email, function (err, result) {
        if (err) {
          console.error(err);
          callback(err);
        }
